...
class Kanzhun(object):
    ...
    def get_comments(self,tree):
        all_comments = tree.xpath('//section[@class="f_l_con"]/section[contains(@class,"public_item review_item wrap_style")]')
    ...
    def fit_url(self,type,param=None):
        if type==1:
            ...
            if self.city:
                url='http://www.kanzhun.com/gsrPage.json?companyId='+self.company_id+'&companyName='+self.company_name+'&pageNum='+str(param)+'&cityCode='+self.city_code(self.city)+'&sortMethod=1&employeeStatus=0'
            else:url='http://www.kanzhun.com/gsrPage.json?companyId='+self.company_id+'&companyName='+self.company_name+'&pageNum='+str(param)+'&cityCode=&sortMethod=1&employeeStatus=0'
        return url
